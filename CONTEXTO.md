...
29. **O EAS não envia o `.env`, e `.easignore` não resolve.** O `.env` está no `.gitignore`, e o EAS monta o pacote de build a partir do que o Git versiona. Criar um `.easignore` que não lista o `.env` **não** foi suficiente: a build de 07/09/2026 saiu sem nenhuma variável e o APK **fechava sozinho ao abrir**, porque `lib/supabase.ts` tem um `throw` no topo quando elas faltam — e um `throw` no import do `App.tsx` derruba o app antes da primeira tela, sem mensagem nenhuma numa build de produção. O caminho que funciona é registrar as variáveis no próprio EAS: `eas env:set --name X --value Y --environment preview --visibility plaintext --scope project --type string --non-interactive`. **O diagnóstico está no output do `eas build`**, na primeira linha: "No environment variables ... found for the preview environment" significa que o APK vai sair quebrado; "Environment variables ... loaded from the preview environment: ..." significa que está certo. Ler essa linha custa dois segundos e evita 20 minutos de build.
